...
def transform_matrix(matrix):
    ...
    for i in range(rows):
        for j in range(cols):
            a_ij = matrix[i, j]
            a_i1j = matrix[i+1, j] if i+1 < rows else 0
            a_ij1 = matrix[i, j+1] if j+1 < cols else 0
            a_i1j1 = matrix[i+1, j+1] if (i+1 < rows and j+1 < cols) else 0
...
input_file = 'sorted_matrices_descending_4.txt'
output_file = 'transformed_matrices.txt'
