...
def plot_graph_from_multiplication_table(M, table_index):
    n = len(M)  # Number of nodes, should be 2 after modification
    ...
    # Define fixed positions for 2 nodes
    pos = {}
    for i in range(n):
    ...
    for u, v in G.edges():
        ...
        if u == v:
            ...
            x, y = pos[u]
            radius = 0.05  # Adjust the radius of the self-loop
            theta1 = 0
    ...
    image_filename = f'graph_{table_index}_2x2.png'
        ...
        for line_number, line in enumerate(file, 1):
            try:
                ...
                if (isinstance(M, list) and all(isinstance(row, list) for row in M)):
                    n = len(M)
                    if n == 2 and all(len(row) == n for row in M):  # Adjust to 2x2
                        multiplication_tables.append(M)
                    else:
                        print(f"Error on line {line_number}: The table must be 2x2.")
                else:
...
def main():
    ...
    filename = 'minizinc_2_mult_cleaned.txt'  # Adjusted for 2x2 table file
