...
def plot_graph_from_multiplication_table(M, table_index):
    n = len(M)  # Number of nodes, should be 4 after modification
    ...
    # Define fixed positions for nodes from 0 to 3 counterclockwise
    pos = {}
    for i in range(n):
    ...
    for u, v in G.edges():
        ...
        if u == v:
            ...
            x, y = pos[u]
            radius = 0.2  # Adjust the radius of the self-loop
            theta1 = 0
    ...
    image_filename = f'graph_{table_index}_4x4.png'
        ...
        for line_number, line in enumerate(file, 1):
            try:
                ...
                if (isinstance(M, list) and all(isinstance(row, list) for row in M)):
                    n = len(M)
                    if n == 4 and all(len(row) == n for row in M):
                        multiplication_tables.append(M)
                    else:
                        print(f"Error on line {line_number}: The table must be 4x4.")
                else:
...
def main():
    ...
    filename = 'minizinc_4_mult_cleaned.txt'
